...
let posiveSentiments = classSentiments.filter((sentiment)=>{
    return sentiment.CategoryLabel === 'Garbage' &&  sentiment.BERT_Sentiment == 'positive'
})
let negativeSentiments = classSentiments.filter((sentiment)=>{
    return sentiment.CategoryLabel === 'Garbage' &&  sentiment.BERT_Sentiment == 'negative'
})
...
let totalTweets = classSentiments.length;
        let positiveTweets = classSentiments.filter(sentiment => sentiment.CategoryLabel === 'Garbage' &&  sentiment.BERT_Sentiment === 'positive');
        let negativeTweets = classSentiments.filter(sentiment => sentiment.CategoryLabel === 'Garbage' &&  sentiment.BERT_Sentiment === 'negative');
...
function GarbageCard(){
    return(
        ...
        <div className='card-container'>
            ...
            <h3>Garbage Based Pie chart of all tweets</h3>
            <Pie data={data} />
            </div>
