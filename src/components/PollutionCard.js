...
let posiveSentiments = classSentiments.filter((sentiment)=>{
    return sentiment.CategoryLabel === 'Pollution' &&  sentiment.BERT_Sentiment == 'positive'
})
let negativeSentiments = classSentiments.filter((sentiment)=>{
    return sentiment.CategoryLabel === 'Pollution' &&  sentiment.BERT_Sentiment == 'negative'
})
...
let totalTweets = classSentiments.length;
        let positiveTweets = classSentiments.filter(sentiment => sentiment.CategoryLabel === 'Pollution' &&  sentiment.BERT_Sentiment === 'positive');
        let negativeTweets = classSentiments.filter(sentiment => sentiment.CategoryLabel === 'Pollution' &&  sentiment.BERT_Sentiment === 'negative');
...
function PollutionCard(){
    return(
        ...
        <div className='card-container'>
            ...
            <h3>Pollution Based Pie chart of all tweets</h3>
            <Pie data={data} />
            </div>
